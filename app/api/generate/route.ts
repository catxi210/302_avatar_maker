...
export async function POST(request: Request) {
  const { main_face_image, content, width, height, presetStyle, characterType } = await request.json();
  if (!main_face_image) {
  ...
  try {
    ...
    const generateResult = await generate({ ...params, main_face_image, width, height })
